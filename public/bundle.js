...
function requestHandler(request, response) {
  ...
  var requestedResource = _path2.default.join(__dirname, // current directory where server.js is found
  '../public', // step out of this directory into the public directory
  _url2.default.parse(request.url).pathname // path to resource requested by client
  );

  // use the exists method of the fs module to check if the requestedResource 
  // exists.
  _fs2.default.exists(requestedResource, function (exists) {
    ...
    if (_fs2.default.statSync(requestedResource).isDirectory()) {
      requestedResource += '/index.html';
    }

    // Finally, we read the requested file (asynchronously) and send it's 
    // content to the client
    _fs2.default.readFile(requestedResource, // path to requested resource
    ...
    function (err, file) {
      ...
      var contentType = contentTypesByExtension[_path2.default.extname(requestedResource)];
